...
## Data & Control Flow
1. **Scrape** – `scripts/automated-scraper.js` runs (GitHub Action, cron, or manual). It calls `HybridUFCService` to fetch events/fights with Wikipedia as the primary source. After fights are built, it automatically enriches fighter win/loss records from Tapology (enabled by default, disable with `TAPOLOGY_ENRICH_RECORDS=false`). Sherdog remains available locally but is disabled in CI due to IP blocking. Enhanced event deduplication prevents duplicate events when multiple sources provide the same event with different naming conventions.
